...
def main():
    ...
    for i in range(1000):
        try:
            url_List = 'http://mobile.nmpa.gov.cn/datasearch/QueryList?tableId=132&searchF=Quick%20SearchK&pageIndex={}&pageSize=1000'.format(
                i)
            ...
            for result in res:
                Id = result['ID']
                url_Detail = 'http://mobile.nmpa.gov.cn/datasearch/QueryRecord?tableId=132&searchF=ID&searchK=%s' % Id
...
def parse2json(content, url_1):
    ...
    sql_insert = "INSERT INTO T_BASE_Country_ManufacturerInfo(MD5, Code, CompanyName, LegalPerson, CEO, HomeAddress, " \
                 "ProductAddress, ScopeDetail, ApprovalDepartment, ApprovedDate, Validuntil, " \
