                    ...
                    if(length>1):
                        ...
                        for td_k in range(1, length):
                            ...
                            sql = "" \
                                  "" \
                                  "" \
                                  "if not exists (select top 1 * from T_BASE_MAMedicareCategoryRegistration  where CodeId='%s' and RegistratioNo='%s'" \
                                  " and Person='%s' and Name='%s' and SpecsNum='%s')  " \
                                  "   begin    " \
                                  "INSERT INTO T_BASE_MAMedicareCategoryRegistration(MD5,CodeId,RegistratioNo, Person, Name, SpecsNum) " \
                                  "VALUES ('%s','%s','%s','%s','%s','%s')" \
                                  "  end   " % (
                                      specificationCode,RegistratioNo, Person, ProName, SpecsNum,
