...
sql_update = 'select * from T_BASE_Country_ProductInfo where type = 1 and IsChina = 0 '
try:
    # 执行查询语句
...
def main():
    ...
    try:
        ...
        for i in range(1000):
            try:
                url_List = 'http://mobile.nmpa.gov.cn/datasearch/QueryList?tableId=27&searchF=Quick%20SearchK&pageIndex={}&pageSize=1000'.format(i)
                ...
                for result in res:
                    Id = result['ID']
                    url_Detail = 'http://mobile.nmpa.gov.cn/datasearch/QueryRecord?tableId=27&searchF=ID&searchK=%s' % Id
...
def parse2json(content,url_1):
    ...
    sql_insert = "" \
                 "if not exists (select top 1 * from T_BASE_Country_ProductInfo where Code = '%s' and type = 1 and IsChina = 0 )" \
                 " begin " \
                 "INSERT INTO T_BASE_Country_ProductInfo(MD5, Code, RegistrantName, RegistrantAddress, FactoryAddress," \
                 "ProxyName, ProxyAddress, ProductName, [Level], Spec, MainComponent, UsageDescription, StorageMethod, " \
        ...
        (content[0].replace('"', ""),url_md5, content[0].replace('"', ""), content[1].replace('"', ""), content[2].replace('"', ""),
         content[3].replace('"', "").replace("\'s", "").replace("\'", "").replace("\\n", "").replace("\\r", ""), content[4].replace('"', "").replace("\'s", "").replace("\'", "").replace("\\n", "").replace("\\r", ""),
