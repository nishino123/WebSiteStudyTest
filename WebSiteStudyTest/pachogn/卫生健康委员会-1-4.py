...
####################查询具体的机构名称######################################
    hospital_items={}
    hospital_items_list=[]
    sql_hospital = ' select *  from Temp_Hospital   where Id > 28974' \
    ...
    try:
        ...
        for row in results_hospital:
            ...
            hospital_items={'Code': Code,
                            'Region': Region,
                            'Hospital': Hospital
                            }
            hospital_items_list.append(hospital_items)

    except Exception as e:
        raise e


    for hospital_item in hospital_items_list:
        ...
        for tr in trs:
            ...
            sql = "INSERT INTO T_BASE_MAMedicareHealthOrg(MD5,Code,Province, ExaminingApprovingOrg, HospitalName,  HospitalLevel, RegistrationNo, Address,Specialty,LegalPerson,ChargePerson,EffectiveTime,DisableTime) " \
                   "VALUES ('%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s')" % (mdf5,code,
                                                          item_dict['province'].rstrip(), item_dict['examing_approving_org'].rstrip(),
                                                          item_dict['hospital'].rstrip(), item_dict['level'].rstrip(), detail_item_dict['registrationNo'].rstrip(), detail_item_dict['adress'].rstrip(),
            detail_item_dict['specialty'].rstrip().replace("\n", "").replace("\r", ""), detail_item_dict['legalPerson'].rstrip(), detail_item_dict['chargePerson'].rstrip(),
            detail_item_dict['effictiveTime'].rstrip(), detail_item_dict['disableTime'].rstrip()
            )
            print('sql: ', sql)
            # try:
