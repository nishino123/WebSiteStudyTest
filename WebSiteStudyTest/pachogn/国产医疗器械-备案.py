...
def main():
    ...
    for i in range(1,1000):
        url_List = 'http://mobile.nmpa.gov.cn/datasearch/QueryList?tableId=165&searchF=Quick%20SearchK&pageIndex={}&pageSize=1000'.format(i)
        ...
        for result in res:
            try:
                Id = result['ID']
                url_Detail = 'http://mobile.nmpa.gov.cn/datasearch/QueryRecord?tableId=165&searchF=ID&searchK=%s' % Id
...
def parse2json(content,url_1):
    ...
    sql_insert = "INSERT INTO T_BASE_Country_ProductInfo(MD5, Code, RegistrantName, RegistrantAddress, FactoryAddress,  " \
                 "ProductName, Spec, Describes, UsageDescription, " \
                 "Remark, ApprovalDepartment, ApprovedDate, Changes,StorageMethod , Type, IsChina) " \
          "VALUES ('%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s')" % \
        (url_md5,
         content[0].replace('"', "").replace("\'s", "").replace("\'", "").replace("\\n", "").replace("\\r", ""),
         content[1].replace('"', "").replace("\'s", "").replace("\'", "").replace("\\n", "").replace("\\r", ""), content[2].replace('"', "").replace('"', "").replace("\'s", "").replace("\'", "").replace("\\n", "").replace("\\r", ""),
