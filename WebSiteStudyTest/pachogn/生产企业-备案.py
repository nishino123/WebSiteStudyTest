...
def main():
    ...
    for i in range(1000):
        try:
            url_List = 'http://mobile.nmpa.gov.cn/datasearch/QueryList?tableId=133&searchF=Quick%20SearchK&pageIndex={}&pageSize=1000'.format(
                i)
            ...
            for result in res:
                Id = result['ID']
                url_Detail = 'http://mobile.nmpa.gov.cn/datasearch/QueryRecord?tableId=133&searchF=ID&searchK=%s' % Id
                ...
                print('第' + str(i) + '条数据： ', ylqx)  # 因为有的cs数据取到的是空，所以才会出现list index out of range,所以不用去管它
...
def parse2json(content, url_1):
    ...
    sql_insert = "INSERT INTO T_BASE_Country_ManufacturerInfo(MD5, Code, CompanyName, LegalPerson, CEO, HomeAddress, " \
