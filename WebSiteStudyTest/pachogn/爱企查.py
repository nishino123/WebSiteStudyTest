...
login = {'user':'15797671998','password':'NOGIZAKA46q!'}
sess.post('https://aiqicha.baidu.com',data=login,headers=afterLogin_headers) #模拟登录  或者传入cookies

def get_company_message(company):
    a = 1
    # 获取查询到的网页内容（全部）
    # search = sess.get('https://aiqicha.baidu.com/search?key={}'.format(company),headers=afterLogin_headers,timeout=10)
    ...
    href = 'https://aiqicha.baidu.com/detail/basicAllDataAjax?pid=52591758479222'  #处于未登录状态 0 异步调用接口 ==》不是狭义上的url
    time.sleep(4)
    # 获取查询到的网页内容（全部）
    details = sess.get(href,headers=afterLogin_headers,timeout=10)
    details.raise_for_status()
    # details.encoding = 'utf-8' #linux utf-8
    text = details.text #字符串转json
    import simplejson
    text = simplejson.loads(text) #直接转过来，解码
